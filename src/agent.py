            ...
            agent_kwargs={
                "system_message": 
                    ...
                    **Step-by-Step Data Analysis Workflow (For Class B queries):**
                    ...
                    Sample_Column_Values tool if a query requires filtering by a specific condition (e.g., 'treatment' or 'patient').
                    ...
                    The output of the SQL_Query tool must be the input for this tool.
                    e.  CSV Report (if requested): If the user explicitly asks for a report or a CSV export, use the 
                    Create_Report tool. The output of the SQL_Query tool must be the input for this tool.
                    f.  Final Response Generation: Synthesize the retrieved data (and the plot/csv report, if applicable) into
                    a concise, clear, and biologically informed answer. Never mention the filename of the plot or csv report that
                    you produced. Your response should directly address the user's question.
